...
function reducer(state, { type, payload }) {
  switch (type) {
    ...
    case ACTIONS.CHOOSE_OPERATION:
      ...
      if (state.operation && state.operation !== payload.operation) { return { ...state, operation: payload.operation } }
...
const INTEGER_ROMATTER = new Intl.NumberFormat("en-us", {maximumFractionDigits: 0, })

function formatOperand(operand) {
  if (operand == null) return; 
  const [integer, decimal] = String(operand).split(".");
  if (decimal == null) return INTEGER_ROMATTER.format(integer); 
  return `${INTEGER_ROMATTER.format(integer)}.${decimal}`;
}
